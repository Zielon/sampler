...
class SMPLlayer(nn.Module):
    def __init__(self, model_path, model_type='smpl', gender='neutral', device=None,
        ...
        **kwargs) -> None:
        ...
        if regressor_path is None:
            regressor_path = str(Path(__file__).parent.parent) + "/assets/J_regressor_body25_smplx.txt"
        ...
        self.register_buffer('faces_tensor',
                             to_tensor(to_np(self.faces, dtype=np.int64),
                                       dtype=torch.long))
        for key in ['J_regressor', 'v_template', 'weights', 'lmk_faces_idx', 'lmk_bary_coords']:
    ...
    def forward(self, poses, shapes, Rh=None, Th=None, expression=None, 
        ...
        if return_verts or not self.use_joints:
            ...
            vertices, joints, T, A, bs = self.lbs(shapes, poses, v_template,
                                self.shapedirs, self.posedirs,
                                self.J_regressor, self.parents,
            ...
            vertices, joints, T, A, bs = self.lbs(shapes, poses, self.j_v_template,
