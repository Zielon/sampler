...
def lbs(
    ...
    return verts, J_transformed, T, A, bs
...
def batch_dqs_blending(A, W, Vs):
    ...
    Rs = A[:, :3, :3]
    ws = torch.sqrt(torch.clamp(Rs[:, 0, 0] + Rs[:, 1, 1] + Rs[:, 2, 2] + 1.0, min=1.0e-6)) / 2.0
    xs = (Rs[:, 2, 1] - Rs[:, 1, 2]) / (4.0 * ws)
    ys = (Rs[:, 0, 2] - Rs[:, 2, 0]) / (4.0 * ws)
    zs = (Rs[:, 1, 0] - Rs[:, 0, 1]) / (4.0 * ws)
    Ts = A[:, :3, 3]
    ...
    b0 = W.unsqueeze(-2) @ torch.cat([ws[:, None], xs[:, None], ys[:, None], zs[:, None]], dim=-1).reshape(
        Bnum, 1, Jnum, 4
    )  # B,V,J,4
    be = W.unsqueeze(-2) @ torch.cat([vDw[:, None], vDx[:, None], vDy[:, None], vDz[:, None]], dim=-1).reshape(
        Bnum, 1, Jnum, 4
    )  # B,V,J,4
    b0 = b0.reshape(-1, 4)
    be = be.reshape(-1, 4)
    ...
    Vs = (
        Vs
        + 2.0 * b0[:, 1:].cross(b0[:, 1:].cross(Vs) + b0[:, :1] * Vs)
        + 2.0 * (b0[:, :1] * be[:, 1:] - be[:, :1] * b0[:, 1:] + b0[:, 1:].cross(be[:, 1:]))
    )
